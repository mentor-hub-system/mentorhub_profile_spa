...
- Developer [SPA Standard Prerequisites](https://github.com/mentor-hub-system/mentorhub/blob/main/DeveloperEdition/standards/spa_standards.md)

## Dependency: `spa_utils` (git install, not the npm registry)

`@mentor-hub-system/mentorhub_spa_utils` is declared in **`package.json`** as a **GitHub git dependency** (`github:mentor-hub-system/mentorhub_spa_utils#main`). `npm install` clones that repository and runs its `prepare` script to build **`dist/`**. There is **no** semver range like `^0.1.0`; bump the branch or tag in **`package.json`** when you want a new revision.
...
**Note**: This template uses `@mentor-hub-system/mentorhub_spa_utils` for reusable components, composables, and utilities. See the [spa_utils README](../spa_utils/README.md) for complete documentation on available components (`AutoSaveField`, `AutoSaveSelect`, `ListPageSearch`), composables (`useResourceList`, `useErrorHandler`, `useRoles`), and utilities (`formatDate`, `validationRules`).
...
This template uses components and composables from `@mentor-hub-system/mentorhub_spa_utils`:
